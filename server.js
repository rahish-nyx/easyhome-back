...
const commissionSchema = new mongoose.Schema({
  ...
  status: { type: String, default: "pending" }, // pending|paid|approved
  screenshotNote: { type: String, default: "" },
  createdAt: { type: Date, default: Date.now },
});
...
const Commission = mongoose.model("Commission", commissionSchema); // ✅ NEW
const Settings = mongoose.model("Settings", settingsSchema); // ✅ NEW
...
async function createDefaultAdmin() {
  try {
    const exists = await Admin.findOne({ email: "admin@easyhome.com" });
    if (!exists) {
      const hashed = await bcrypt.hash("admin123", 10);
      await Admin.create({ email: "admin@easyhome.com", password: hashed });
...
app.post("/admin/city", adminMiddleware, async (req, res) => {
  try {
    ...
    res.json(await City.create({ name, lat, lng }));
  } catch {
    res.status(500).json({ error: "Failed" });
  }
});
  ...
  try {
    ...
    try {
      await transporter.sendMail({
        ...
        html: `<div style="font-family:Arial;padding:20px;"><h2 style="color:#ff3c00;">🏠 EasyHome</h2><h3 style="color:#28a745;">✅ Account Approved!</h3><p>Hi <strong>${worker.name}</strong>, your account has been approved. Login to start accepting jobs!</p></div>`,
      });
    } catch (e) {
...
app.put("/admin/worker/urgent/:id", adminMiddleware, async (req, res) => {
  try {
    ...
    await worker.save();
    res.json({
      message: `Worker ${worker.isUrgent ? "added to" : "removed from"} Need Now`,
...
app.get("/subscription/upi", async (req, res) => {
  try {
    const upiFromSettings = await getSetting("adminUpi", adminSubscriptionUPI);
    res.json({ upi: upiFromSettings });
  } catch {
...
app.get("/subscription/my", authMiddleware, async (req, res) => {
  try {
    const sub = await Subscription.findOne({ userId: req.user.id }).sort({
      createdAt: -1,
    });
    res.json(sub || null);
  } catch {
    ...
    try {
      ...
      try {
        await transporter.sendMail({
          ...
          html: `<div style="font-family:Arial;padding:20px;max-width:400px;"><h2 style="color:#ff3c00;">🏠 EasyHome Premium</h2><h3 style="color:#28a745;">✅ Your subscription is active!</h3><p>Hi <strong>${sub.userName}</strong>, welcome to EasyHome Premium!</p><p>Valid until: <strong>${end.toDateString()}</strong></p><p>Enjoy your premium benefits 🚀</p></div>`,
        });
      } catch (e) {
...
app.get("/settings", async (req, res) => {
  try {
    const subscriptionPrice = await getSetting("subscriptionPrice", 199);
    const subscriptionBenefitsCustomer = await getSetting(
    ...
    const subscriptionBenefitsWorker = await getSetting(
    ...
    const qrCodeUrl = await getSetting("qrCodeUrl", "");
    const adminUpi = await getSetting("adminUpi", adminSubscriptionUPI);
    res.json({
      subscriptionPrice,
      subscriptionBenefitsCustomer,
      subscriptionBenefitsWorker,
      ...
      if (subscriptionBenefitsCustomer) {
        const b =
          typeof subscriptionBenefitsCustomer === "string"
            ...
            : subscriptionBenefitsCustomer;
        await setSetting("subscriptionBenefitsCustomer", b);
      }
      if (subscriptionBenefitsWorker) {
        const b =
          typeof subscriptionBenefitsWorker === "string"
            ...
            : subscriptionBenefitsWorker;
        await setSetting("subscriptionBenefitsWorker", b);
      }
      if (adminUpi) {
...
// Worker submits commission payment
...
// Get commission status for a booking
...
app.put("/admin/commission/reject/:id", adminMiddleware, async (req, res) => {
  try {
    ...
    res.json({ message: "Rejected — worker must pay again" });
  } catch {
    res.status(500).json({ error: "Failed" });
  }
});
...
app.put("/booking/:id/set-price", authMiddleware, async (req, res) => {
  try {
    ...
    const workerReceives = Number(price) - commission;
    const updated = await Booking.findByIdAndUpdate(
      req.params.id,
      { workerSetPrice: Number(price), commission, workerReceives },
      { new: true },
    );
    res.json(updated);
  } catch {
...
app.put("/booking/:id/confirm-price", authMiddleware, async (req, res) => {
  try {
    ...
    const confirmedPrice = Number(price);
    const updated = await Booking.findByIdAndUpdate(
      req.params.id,
      { confirmedPrice, finalPrice: confirmedPrice, finalPriceConfirmed: true },
      { new: true },
    );
    res.json(updated);
  } catch {
...
app.get("/worker/earnings", authMiddleware, async (req, res) => {
  try {
    ...
    for (let i = 11; i >= 0; i--) {
      ...
      months.push({ key, label, amount: monthlyMap[key] || 0 });
    }
    const totalEarnings = jobs.reduce((s, j) => s + (j.finalPrice || 0), 0);
    const thisMonth = months[months.length - 1].amount;
    res.json({ months, totalEarnings, thisMonth });
  } catch {
    res.status(500).json({ error: "Failed" });
  }
});
...
app.get("/worker/profile", authMiddleware, async (req, res) => {
  try {
    ...
    const earnings = acceptedJobs.reduce(
      (sum, j) => sum + (j.finalPrice || 0),
      0,
    );
    res.json({ ...worker.toObject(), acceptedJobs, earnings });
  } catch {
    res.status(500).json({ error: "Server error" });
  }
});
...
app.get("/urgent", async (req, res) => {
  try {
    ...
    oneMonthAgo.setMonth(oneMonthAgo.getMonth() - 1);
    res.json(
      await Urgent.find({ createdAt: { $gte: oneMonthAgo } }).sort({ _id: -1 }),
    );
  } catch {
    res.status(500).json({ error: "Failed" });
  }
...
app.get("/urgent-workers", async (req, res) => {
  try {
    res.json(
      await Worker.find({ isUrgent: true, approved: true }, "-password").sort({
        isSubscribed: -1,
        rating: -1,
      }),
    );
...
app.get("/workers", async (req, res) => {
  try {
    res.json(
      await Worker.find({ approved: true }, "-password").sort({
        isSubscribed: -1,
        rating: -1,
      }),
    );
...
app.post(
  ...
  async (req, res) => {
    try {
      let price = Number(req.body.pricePerHour) || 100;
...
// ✅ UPDATED — payment route creates commission for non-premium
app.put("/booking/:id/payment", authMiddleware, async (req, res) => {
  try {
    ...
    // Auto-create commission for non-premium workers
...
app.get("/nearby-jobs", authMiddleware, async (req, res) => {
  try {
